...
export class MixerApp extends FormApplication {
    ...
    async getData() {
        ...
        return {
            ...
            master: masterData,
            players: Array.from(game.users, function(entry) {if (entry._id != game.userId) return {name: entry.name, active: entry.active, _id: entry._id}}),
    ...
    activateListeners(html) {
        ...
        channelBox.on('drop', (event)=>{
            ...
            if (data.type == 'Playlist') {
                ...
                data = {
                    ...
                    playlist: data.uuid.split('.')[1],
                    sound: data.uuid.split('.')[3]
                }
            }
        ...
        sbButton.on('drop', (event)=>{
            ...
            if (data.type == 'Playlist') {
                data = {
                    type: "playlist_multi",
                    playlist: data.uuid.split('Playlist.')[1]
                }
            }
            else if (data.type == 'PlaylistSound') {
                data = {
